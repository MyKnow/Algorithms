// [Int] X (immutable): 구해야 하는 분수의 index (범위 : 1 <= X <= 10,000,000)
let X: Int = Int(readLine() ?? "1") ?? 1

// [Int] nowNumber (mutable): 현재 X의 라인 내 위치
var nowNumber: Int = X

// [Int] index (mutable): X번째 분수가 속해있는 index
var index: Int = 1
...
if index % 2 == 0 {
    print("\(nowNumber)/\(index - nowNumber + 1)")
} else {
    print("\(index - nowNumber + 1)/\(nowNumber)")
}