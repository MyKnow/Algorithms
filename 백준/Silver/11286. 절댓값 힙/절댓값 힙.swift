struct MinHeap {
    ...
    // Min-heap: returns true if the parent node has higher priority than the child node.
    // Priority is based on absolute value; if equal, smaller actual value comes first.
    @inline(__always)
